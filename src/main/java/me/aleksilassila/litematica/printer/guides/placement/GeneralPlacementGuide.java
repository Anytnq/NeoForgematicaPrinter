...
public class GeneralPlacementGuide extends PlacementGuide {
  ...
  protected Optional<Direction> getLookDirection() { return Optional.empty(); }

  protected boolean getRequiresSupport() { return false; }

  protected boolean getRequiresExplicitShift() { return false; }

  protected Vec3d getHitModifier(Direction validSide) {
    return new Vec3d(0, 0, 0);
  }

  protected boolean allowAirPlace() {
    return Configs.ALLOW_AIR_PLACE.getBooleanValue() && !getRequiresSupport();
  }

  private Optional<Direction> getValidSide(SchematicBlockState state) {
    boolean printInAir = allowAirPlace();
    ...
    for (Direction side : sides) {
      if (printInAir && !getRequiresSupport()) {
        ...
        if (getProperty(neighborState.currentState, SlabBlock.TYPE)
    ...
    return validSides.isEmpty() ? Optional.empty()
                                : Optional.of(validSides.getFirst());
  }
  ...
  private Optional<Vec3d> getHitVector(SchematicBlockState state) {
    return getValidSide(state).map(
        side
        -> Vec3d.ofCenter(state.blockPos)
  ...
  getPlacementContext(ClientPlayerEntity player) {
    try {
      ...
      if (validSide.isEmpty() && allowAirPlace()) {
        Direction clickSide =
            lookDirection.map(Direction::getOpposite).orElse(Direction.UP);
        Vec3d airPlaceHit =
            Vec3d.ofCenter(state.blockPos).add(getHitModifier(clickSide));
        BlockHitResult airPlaceHitResult =
            new BlockHitResult(airPlaceHit, clickSide, state.blockPos, false);
        return new PrinterPlacementContext(
            player, airPlaceHitResult, requiredItem.get(), requiredSlot,
            lookDirection.orElse(null), requiresShift);
      }

      if (validSide.isEmpty() || hitVec.isEmpty())
        return null;

      BlockHitResult blockHitResult =
          new BlockHitResult(hitVec.get(), validSide.get().getOpposite(),
                             state.blockPos.offset(validSide.get()), false);

      return new PrinterPlacementContext(
          player, blockHitResult, requiredItem.get(), requiredSlot,
          lookDirection.orElse(null), requiresShift);
    } catch (Exception e) {
