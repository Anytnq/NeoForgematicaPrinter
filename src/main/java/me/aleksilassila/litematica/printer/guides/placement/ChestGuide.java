...
public class ChestGuide extends GeneralPlacementGuide {
  public ChestGuide(SchematicBlockState state) { super(state); }
  ...
  protected List<Direction> getPossibleSides() {
    ChestType targetType =
        getProperty(targetState, ChestBlock.CHEST_TYPE).orElse(null);
    Direction targetFacing =
        getProperty(targetState, ChestBlock.FACING).orElse(null);
    ...
    for (Direction direction : Direction.values()) {
      if (targetType == ChestType.SINGLE &&
          !willConnectToSide(state, direction)) {
        sides.add(direction);
      } else if (wantsToConnectToSide(state, direction) &&
                 willConnectToSide(state, direction)) {
        sides.add(direction);
      }
    }
    ...
    if (sides.isEmpty()) {
      for (Direction direction : Direction.values()) {
        if (!wantsToConnectToSide(state, direction) &&
            !willConnectToSide(state, direction)) {
          sides.add(direction);
        }
      }
  ...
  private boolean willConnectToSide(SchematicBlockState state,
                                    Direction neighborDirection) {
    BlockState neighbor = state.offset(neighborDirection).currentState;
    ChestType neighborType =
        getProperty(neighbor, ChestBlock.CHEST_TYPE).orElse(null);
    Direction neighborFacing =
        getProperty(neighbor, ChestBlock.FACING).orElse(null);
    Direction facing =
        getProperty(state.targetState, ChestBlock.FACING).orElse(null);

    if (neighborType == null || neighborFacing == null || facing == null)
      return false;

    if (facing.getAxis() == neighborDirection.getAxis() ||
        neighborDirection.getAxis() == Direction.Axis.Y)
      return false;
  ...
  private boolean wantsToConnectToSide(SchematicBlockState state,
                                       Direction direction) {
    ChestType type =
        getProperty(state.targetState, ChestBlock.CHEST_TYPE).orElse(null);
    Direction facing =
        getProperty(state.targetState, ChestBlock.FACING).orElse(null);
    if (type == null || facing == null || type == ChestType.SINGLE)
      return false;

    Direction neighborDirection = type == ChestType.LEFT
