...
public class PrepareAction extends Action {
  ...
  public PrepareAction(PrinterPlacementContext context, float yaw,
                       float pitch) {
    this.context = context;
  ...
  public void send(MinecraftClient client, ClientPlayerEntity player) {
    ...
    if (itemStack != null && client.interactionManager != null) {
      ...
      if (player.getAbilities().creativeMode) {
        ...
        client.interactionManager.clickCreativeStack(
            player.getStackInHand(Hand.MAIN_HAND), 36 + inventory.selectedSlot);
      } else if (slot != -1) {
    ...
    if (context.shouldSneak) {
      ...
      player.networkHandler.send(new ClientCommandC2SPacket(
          player, ClientCommandC2SPacket.Mode.PRESS_SHIFT_KEY));
    } else {
      player.input.sneaking = false;
      player.networkHandler.send(new ClientCommandC2SPacket(
          player, ClientCommandC2SPacket.Mode.RELEASE_SHIFT_KEY));
    }
  }
